...
fact NoConflictingGreen {
  all s1, s2: Signal |
    s1.currentPhase = Green and s2.currentPhase = Green
}
...
assert NoConflictingGreenAssert {
  all s1, s2: Signal |
    s1 != s2 and
...
}assert EmergencyPriorityAssert {
  all e: EmergencyVehicle |
    one s: Signal |
      s.direction = e.priorityRoute and
      s.currentPhase = Green
...
assert ValidSignalDirection {
  all s: Signal | s.direction in Direction
}


check NoConflictingGreenAssert for 5
